...
static const ControlKey KeyChoice[] = 
{
   ...
   { "Ins"     , IDS_INS,     MR_KDB, VK_INSERT },  // 7
   { "Del"     , IDS_DEL,     MR_KDB, VK_DELETE },  // 8
   { "Enter"   , IDS_ENTER,   MR_KDB, VK_RETURN },  // 9
   { "CapsLock", IDS_CAPS,    MR_KDB, VK_CAPITAL},  // 10
...
unsigned long MR_GameThread::Loop( LPVOID pThread )
{
   ...
   while( 1 )
   {
      ...
      lThis->mGameApp->ReadAssyncInputControler();
...
void MR_GameApp::LoadRegistry()
{
   ...
   // if( lError == ERROR_SUCCESS )
   {
      ...
      if( RegQueryValueEx(  lProgramKey,
                            ...
                            NULL,
                            lControlBuffer,
                            &lControlBufferSize ) == ERROR_SUCCESS )
      {
         ...
         mMotorOn2   = lControlBuffer[7];
         mRight2     = lControlBuffer[8];
         mLeft2      = lControlBuffer[9];
         mJump2      = lControlBuffer[10];
         mFire2      = lControlBuffer[11];
         mBreak2     = lControlBuffer[12];
         mWeapon2    = lControlBuffer[13];
      }
      ...
      if( RegQueryValueEx(  lProgramKey,
                            ...
                            0,
                            NULL,
                            (MR_UInt8*)lVideoSetting,
                            &lVideoSettingSize ) == ERROR_SUCCESS )
      {
                            ...
                            &lBufferSize ) == ERROR_SUCCESS )
      {
         ...
         {
            static unsigned char sKeyBuffer[20] = { 0x57,0xcc,0x3d,0x21,0xcc,0x20,0xd7,0x34,0x62,0x01,0x50,0x0e,0x65,0x90,0xe9,0x9b,0x39,0x70,0x5b,0x99 };
...
void MR_GameApp::SaveRegistry()
{
   ...
   */
   ...
   if( lError == ERROR_SUCCESS )
   {
      ...
      lControlBuffer[7]  = mMotorOn2 ;
      lControlBuffer[8]  = mRight2   ;
      lControlBuffer[9]  = mLeft2    ;
      lControlBuffer[10] = mJump2    ;
      lControlBuffer[11] = mFire2    ;
      lControlBuffer[12] = mBreak2   ;
      lControlBuffer[13] = mWeapon2  ;
      ...
      if( RegSetValueEx(  lProgramKey,
                          ...
                          REG_BINARY,
                          lControlBuffer,
                          sizeof( lControlBuffer )    ) != ERROR_SUCCESS )
      {
         lReturnValue = FALSE;
         ASSERT( FALSE );
      }
      ...
      if( RegSetValueEx(  lProgramKey,
                          ...
                          REG_BINARY,
                          (MR_UInt8*)lVideoSetting,
                          sizeof( lVideoSetting )    ) != ERROR_SUCCESS )
      {
         lReturnValue = FALSE;
         ASSERT( FALSE );
      }
      ...
      if( RegSetValueEx(    lProgramKey,
                            ...
                            0,
                            REG_SZ,
                            (const unsigned char*)(const char*)mNickName,
                          ...
                          sizeof( mDisplayFirstScreen )    ) != ERROR_SUCCESS )
      {
         ...
         if( RegSetValueEx(    lProgramKey,
                               ...
                               0,
                               REG_SZ,
                               (const unsigned char*)(const char*)mOwner,
         ...
         if( RegSetValueEx(    lProgramKey,
                               ...
                               0,
                               REG_SZ,
                               (const unsigned char*)(const char*)mCompany,
         ...
         if( gKeyEncriptFilled )
         {
            if( RegSetValueEx(    lProgramKey,
                                  ...
                                  0,
                                  REG_BINARY,
                                  (const unsigned char*)&gKeyEncript,
...
void MR_GameApp::DisplaySite()
{
   ...
   LoadURLShortcut( mMainWindow, "GrokkSoft.url" );
}
...
BOOL MR_GameApp::IsFirstInstance()const
{
  ...
  if( lPrevAppWnd != NULL )
  {
    ...
    // BringWindowToTop( lPrevAppWnd );
    SetForegroundWindow( lPrevAppWnd );
    // If iconic, restore the main window.
    // if( IsIconic( lPrevAppWnd ) )
    {
...
void MR_GameApp::RefreshTitleBar()
{
   if( mMainWindow != NULL )
   {
      if( gKeyFilled )
      {
         SetWindowText( mMainWindow, MR_LoadString( IDS_REG_CAPTION ) );
      }
      else
      {
         SetWindowText( mMainWindow, MR_LoadString( IDS_DEMO_CAPTION ) );
      }
   }
}
