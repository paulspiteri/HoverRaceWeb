         ...
         case MRNM_CREATE_AUTO_ELEM:
            {
               ...
               if( lNewElement != NULL )
               {
                  ...
                  if( (mMainCharacter1 != NULL)&&(lNetworkDelay > 50) )
                  {
                     ...
                     if( lVisible )
                     {
                        mSession.SimulateLateElement( lElementHandle, lNetworkDelay+50, lRoom ); // the 50ms extra is only a small bonus
                     }
                  }
               }
            }
...
{
   pTime += 0;//;/; mNetInterface.GetLagFromServer();
      ...
      for( int lCounter = 0; lCounter < WebPeerInterface::eMaxClient; lCounter++ )
      {
         if( mClientCharacter[ lCounter ] != NULL )
         {
            ...
            if( mClientCharacter[ lCounter ]->mRoom == mMainCharacter1->mRoom )
            {
                  if( mMainCharacter1->mNetPriority )
                  {
                     mLastSendElemStateTime[lCounter] -= 100; // to keep priority state even when this broadcast will be finish
                     lPriorityLevel[ lCounter ] += 200;       // major priority boost
                  }
                  else
                  {
...
void MR_NetworkSession::InsertHitEntry( PlayerResult* pEntry )
{
   ...
   while( *lPtr != NULL )
   {
      if((pEntry->mNbGoodShot-pEntry->mNbTimeHit) >
         ((*lPtr)->mNbGoodShot-(*lPtr)->mNbTimeHit) )
      {
         break;
      }
      lPtr = &((*lPtr)->mNextHitResult);
