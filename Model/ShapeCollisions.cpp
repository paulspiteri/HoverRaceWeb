...
static BOOL MR_AreLineCrossing( MR_Int32 pAX0, MR_Int32 pAY0, 
                                MR_Int32 pAX1, MR_Int32 pAY1,
                                MR_Int32 pBX0, MR_Int32 pBY0,
                                MR_Int32 pBX1, MR_Int32 pBY1 );
...
BOOL MR_GetPolygonInclusion( const MR_PolygonShape&     pPolygon,
                             const MR_2DCoordinate&     pPosition    )
{
   BOOL lAnswer = TRUE;

   // Verify that the point is inside the bounding box of the polygon
   if( (pPosition.mX < pPolygon.XMin()) || 
       (pPosition.mX > pPolygon.XMax()) || 
       (pPosition.mY < pPolygon.YMin()) || 
       (pPosition.mY > pPolygon.YMax())   )
   {
      lAnswer = FALSE;
   }
   else
...
void MR_LineRoomContact( const MR_LineSegmentShape* pActor, const MR_PolygonShape* pRoom, MR_RoomContactSpec& pAnswer )
{
   ...
   for( int lCounter = 0; pAnswer.mTouchingRoom && (lCounter<lVertexCount); lCounter++ )
   {
      ...
      if( MR_AreLineCrossing( pRoom->X(lCounter), pRoom->Y(lCounter), 
                              pRoom->X(lP1), pRoom->Y(lP1),
                              pActor->X0(), pActor->Y0(),
                              pActor->X1(), pActor->Y1() ) )
      {
         MR_AddContactWall( lCounter, pAnswer );
   ...
   {
      if( MR_GetPolygonInclusion( *pRoom,
                                  MR_2DCoordinate( pActor->X0(), pActor->X1() ) ) )
      {
         pAnswer.mTouchingRoom = TRUE;
      }
   }
...
void MR_PolygonRoomContact( const MR_PolygonShape* pActor, const MR_PolygonShape* pRoom, MR_RoomContactSpec& pAnswer )
{
          ...
          &&( pActor->XMax() >= std::min( pRoom->X( lP0Room ), pRoom->X( lP1Room ) ) ) )
      {
             ...
             &&( pActor->YMax() >= std::min( pRoom->Y( lP0Room ), pRoom->Y( lP1Room ) ) ) )
         {
            for( int lP0Actor = 0; lP0Actor < lActorSides; lP0Actor++ )
            {
               ...
               if( MR_AreLineCrossing( pRoom->X(lP0Room),   pRoom->Y(lP0Room), 
                                       pRoom->X(lP1Room),   pRoom->Y(lP1Room),
                                       pActor->X(lP0Actor), pActor->Y(lP0Actor),
                                       pActor->X(lP1Actor), pActor->Y(lP1Actor) ) )
               {
   ...
   {
      if( MR_GetPolygonInclusion( *pRoom,
                                  MR_2DCoordinate( pActor->X(0), pActor->Y(0) ) ) )
      {
         pAnswer.mTouchingRoom = TRUE;
      }
   }   
...
BOOL MR_Is1Outside0( const MR_PolygonShape* pActor0, const MR_PolygonShape* pActor1 )
{
   ...
   for( int lVertex1 = 0; 
        !lReturnValue &&(lVertex1 < lVertex1Count); 
        lVertex1++                                   )
   {
      lReturnValue = TRUE;
      ...
      {
         lReturnValue = MR_IsOnLeft( MR_2DCoordinate( pActor0->X( lVertex0 ), pActor1->Y( lVertex0 ) ),
                                     MR_2DCoordinate( pActor0->X( lVertex1 ), pActor1->Y( lVertex1 ) ),
                                     MR_2DCoordinate( pActor0->X( (lVertex1+1)%lVertex1Count ), pActor1->X( (lVertex1+1)%lVertex1Count ) ) );
      }
   }
...
BOOL MR_AreLineCrossing( MR_Int32 pAX0, MR_Int32 pAY0, 
                         MR_Int32 pAX1, MR_Int32 pAY1,
                         MR_Int32 pBX0, MR_Int32 pBY0,
                         MR_Int32 pBX1, MR_Int32 pBY1 )
{
